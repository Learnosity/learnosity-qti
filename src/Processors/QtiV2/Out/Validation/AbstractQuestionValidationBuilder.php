...
abstract class AbstractQuestionValidationBuilder
{
    ...
    protected function buildResponseProcessing($validation, $isCaseSensitive = true)
    {
        ...
        if (method_exists($validation, 'get_valid_response') && method_exists($validation->get_valid_response(), 'get_score')) {
            $score = $validation->get_valid_response()->get_score();
        } else {
            $score = null;
        }
        ...
        if (in_array($questionType, Constants::$questionTypesWithMappingSupport)) {
            ...
            $template = ($scoringType === 'exactMatch' && $score === 1) ? Constants::RESPONSE_PROCESSING_TEMPLATE_MATCH_CORRECT : Constants::RESPONSE_PROCESSING_TEMPLATE_MAP_RESPONSE;
