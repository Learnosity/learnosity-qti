...
namespace LearnosityQti\Utils\Xml;

class EntityUtil
{
    public static function convertNamedEntitiesToHexInString($xml)
    {
        $HTML401NamedToNumeric = static::getNumericEntitiesByNamedEntity();

        foreach ($HTML401NamedToNumeric as $namedEntity => $numericEntity) {
            $xml = str_replace($namedEntity, $numericEntity, $xml);
        }

        return $xml;
    }

    protected static function getNumericEntitiesByNamedEntity()
    {
        return array(
